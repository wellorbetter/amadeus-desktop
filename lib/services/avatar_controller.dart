...
import 'pet_asset_server.dart';
import 'pet_config.dart';
import 'pet_model.dart';
import 'pet_logger.dart';
import 'pet_webview_surface.dart';

/// Cross-platform Live2D avatar surface (WebView2 on Windows, WKWebView on macOS).
class AvatarController {
  /// 底部固定预留的聊天输入区高度（窗口贴合模型时算入窗口高度）。
  static const double inputReserve = 84.0;

  final PetWebViewSurface _surface = PetWebViewSurface();
  final PetAssetServer _assets = PetAssetServer();
  static const MethodChannel _windowChannel = MethodChannel('timepet/window');

  Widget get view => _surface.widget;
  ...
  // Current user-selected model served by the loopback-only asset origin.
  String? _modelFile;
  String? _modelDir;
  ...
  Future<void> initialize() async {
    ...
    PetLog.i('avatar: initialize start');
    final model = PetModel.resolve();
    if (model != null) {
      _modelFile = model.file;
      _modelDir = model.dir;
      PetLog.i('avatar: model selected file=${model.file} dir=${model.dir}');
    } else {
      PetLog.w('avatar: no model resolved, fallback hint will show');
    }
    await _assets.start(modelDirectory: _modelDir);
    final modelParam = _modelFile == null
        ? ''
        : '?model=${Uri.encodeComponent(_modelFile!)}';
    await _surface.initialize(
      initialUrl: '${_assets.origin}/web/avatar.html$modelParam',
      onMessage: _handleWebMessage,
      onNavigationCompleted: _handleNavigationCompleted,
      onError: (error) => PetLog.e('avatar: webview error=$error'),
    );
    PetLog.i('avatar: loadUrl done');
    _initialized = true;
    _pollStatus();
  }
  ...
  void _handleNavigationCompleted() {
    _navCount++;
    PetLog.i('avatar: navigation completed #$_navCount');
    unawaited(_afterNavigation());
  }

  void _handleWebMessage(String text) {
    if (text.contains('drag')) {
      ...
      if (m != null) {
        ...
        unawaited(
          _windowChannel
              .invokeMethod('setHitRegion', [x, y, w, h])
              .catchError((_) => null),
        );
        onModelFit?.call(x, y, w, h, shrink);
      }
    } else {
      PetLog.i('avatar: webMessage=$text');
    }
  }
  ...
  Future<void> _afterNavigation() async {
    final n = _navCount;
    PetLog.i('avatar: after-navigation #$n start');
    await Future.delayed(const Duration(milliseconds: 600));
    if (!_initialized) return;
    try {
      // 1) 外观兜底：与配置不一致时重新推送（scale 变化会触发页面 reinit）
      await applyAppearance();
      PetLog.i('avatar: after-navigation #$n appearance pushed');
      ...
      if (fit != null) {
        ...
        PetLog.i(
          'avatar: after-navigation #$n refit ${fit.w}x${fit.h}@${fit.x},${fit.y}',
        );
      }
      ...
      if (n > 1) {
        ...
        for (var i = 0; i < 6 && !ok; i++) {
          ...
          if (!ok) {
            PetLog.i('avatar: after-navigation #$n waiting appearance i=$i');
          }
        }
        await reveal();
        PetLog.i('avatar: after-navigation #$n fallback reveal ok=$ok');
      }
    } catch (e) {
      PetLog.e('avatar: after-navigation error: $e');
    }
  }
  ...
  Future<bool> _appearanceMatches() async {
    try {
      final r = await _surface.executeScript(
      ...
      if (r is String && r.startsWith('{')) {
        ...
        PetLog.i(
          'avatar: appearance check scale=$scale vOff=$vOff op=$op ok=$ok',
        );
        return ok;
      }
    } catch (e) {
      PetLog.e('avatar: appearance check error: $e');
    }
    return false;
  }
  ...
  Future<void> _pollStatus() async {
    for (var i = 1; i <= 15; i++) {
      ...
      if (!_initialized) return;
      try {
        final log = await _surface.executeScript(
          'window.__petLog ? JSON.stringify(window.__petLog) : "NO_PET_LOG"',
        );
        final canvas = await _surface.executeScript(
          '!!document.querySelector("canvas")',
        );
        PetLog.i('avatar: poll#$i canvas=$canvas log=$log');
        if (canvas == true || canvas == 'true') {
          PetLog.i('avatar: canvas ready');
          try {
            final diag = await _surface.executeScript(
              'window.pet && window.pet.diag ? window.pet.diag() : "NO_DIAG"',
            );
            PetLog.i('avatar: diag=$diag');
          } catch (e) {
            PetLog.e('avatar: diag error: $e');
          }
          break;
        }
      } catch (e) {
        PetLog.e('avatar: poll error: $e');
      }
    }
  }
  ...
  Future<void> setModel() async {
    ...
    if (model != null) {
      _modelFile = file;
      _modelDir = dir;
      _assets.modelDirectory = dir;
      PetLog.i('avatar: model source updated file=$file dir=$dir');
    } else {
      _modelFile = null;
      _modelDir = null;
      _assets.modelDirectory = null;
      PetLog.w('avatar: model removed');
    }
    final jsFile = _modelFile == null
        ? 'null'
        : "'/model/${Uri.encodeComponent(_modelFile!)}'";
  ...
  Future<void> applyAppearance() async {
    ...
    PetLog.i(
      'avatar: applyAppearance w=${cfg.displayWidth} h=${cfg.displayHeight} '
  ...
  Future<void> fitToModel(int x, int y, int w, int h) async {
    ...
    PetLog.i('avatar: fitToModel x=$x y=$y w=$w h=$h');
  }
  ...
  Future<void> requestFit({bool allowShrink = false}) async {
    ...
    PetLog.i('avatar: requestFit allowShrink=$allowShrink');
  }
  ...
  Future<void> setInputHitRegion(bool enabled) async {
    try {
      ...
      // Native hit-region optimization is currently Windows-only. macOS keeps
      // the transparent window interactive and still supports drag/click.
    }
  }
  ...
  Future<void> reload() async {
    ...
    final modelParam = _modelFile == null
        ? ''
        : '?model=${Uri.encodeComponent(_modelFile!)}';
    final url = '${_assets.origin}/web/avatar.html$modelParam';
    PetLog.i('avatar: reload current model=${_modelFile ?? 'none'}');
    await _surface.loadUrl(url);
  }
  ...
  Future<void> _exec(String script) async {
    if (!_initialized) return;
    try {
      await _surface.executeScript(script);
    } catch (_) {
      // WebView 未就绪时静默忽略
    }
  }
